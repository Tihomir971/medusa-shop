<script lang="ts">
	...
	const handleSelection = (option: any, value: string) => {
		...
		selectedVariantId =
			findVariant(selectedOptions, product) ||
			product.variants[
				product.variants.findIndex((variant: any) =>
					variant.options.find((option: any) => option.value === value)
				)
			].id;
		selectedOptions = findSelectedOptions(selectedVariantId, product);
		/*  $v = selectedVariantId */
				...
				<div class="mt-4">
					<div class="flex flex-wrap">
						{#each option.filteredValues as value}
							{#if selectedOptions && selectedOptions.length > 0 && value === selectedOptions[option.id]}
								<button
									type="button"
